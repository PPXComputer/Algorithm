...
public:
  explicit LRUCache(int capacity)
      : m_double_list{}, m_map(capacity), m_capcaity(capacity) {}
...
public:
  ...
  std::vector<int> getNewsFeed(int userId) {
    ...
    for (int i = m_posts.size() - 1; i >= 0 && count != 10; --i) {
      ...
      if ((beforeUserId != userId) ||
          (m_userLink[userId].find(beforeUserId) != m_userLink[userId].end())) {
  ...
  std::unordered_map<int, std::unordered_set<int>> m_userLink =
      std::unordered_map<int, std::unordered_set<int>>(20);
