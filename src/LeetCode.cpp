...
void LeetCode::intervalIntersection()
{
	...
	const auto answerEnum = [](vector<vector<int>>& firstList, vector<vector<int>>& secondList) ->vector<vector<int>> {


		const auto answerForward = [&](int first_pos, int second_pos)->std::optional<vector<int>> {
			//得到当前的第pos的数据之间的交集
			vector<int> first_list = firstList[first_pos];
			vector<int> second_list = secondList[second_pos];
			// 从当前的交集判断当前的形式
			if (first_list[0] == second_list[0] and second_list[1] == first_list[1])return { {first_list[0],first_list[1]} };
			if (first_list[0] == second_list[1])return { {first_list[0],first_list[0]} };
			else if (first_list[1] == second_list[0])return { {first_list[1],first_list[1]} };
			else if (first_list[0] > second_list[1] or first_list[1] <= second_list[0])return {};
			else if (first_list[1] < second_list[1] and first_list[0] <= second_list[0]) return { {second_list[0],first_list[1]} };
			else if (second_list[1] < first_list[1] and first_list[0] >= second_list[0]) return { {first_list[0],second_list[1]} };
			else if (second_list[0] > first_list[0] and second_list[1] <= first_list[1])return { {second_list[0],second_list[1]} };
			else if (second_list[0] < first_list[0] and second_list[1] >= first_list[1])return { {first_list[0],first_list[1]} };
			return {};
		};
		vector<vector<int>>::size_type first_size = firstList.size();
		vector<vector<int>>::size_type second_size = secondList.size();
		vector<vector<int>> result;
		result.reserve(std::min(first_size, second_size));
		for (size_t i = 0; i < first_size; i++)
		{
			for (size_t j = 0; j < second_size; j++)
			{
				auto&& data = answerForward(i, j);
				if (data.has_value()) {
	...
	const auto answerImpl = [](vector<vector<int>>& firstList, vector<vector<int>>& secondList) {
		...
		result.reserve(std::min(first_size, second_size));
		int i = 0, j = 0;
		for (; i < first_size and j < second_size;)
		{
			// 只要判断当前 i 和j的移动即可就是了
			if (firstList[i][1] < secondList[j][0]) {
				...
				int first = secondList[j][0] >= firstList[i][0] ? secondList[j][0] : firstList[i][0];
				int second = firstList[i][1] <= secondList[j][1] ? firstList[i][1] : secondList[j][1];
