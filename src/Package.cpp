...
void Package::pack02() {
...

  {
    ...
    function<void(int, const string &)> impl = [&](int index,
                                                   const string &cur) {
      if (index == data.size()) {
      ...
      if (index != data.size() - 1 and data[index] <= '2' and
          data[index + 1] <= '6') {
        char curChar = (data[index] - '1') * 10 + (data[index + 1] - '1' + 'A');
  ...
  {
    function<int(int, const string &)> impl2 = [&](int index,
                                                   const string &cur) {
      if (index == data.size()) {
        return 1;
      }
  ...
  };
  {
    ...
    for (size_t i = 1; i < str1.size(); i++) {
      for (size_t j = 1; j < str2.size(); j++) {
        ...
        help[i][j] = std::max({endWithIJ, help[i - 1][j], help[i][j - 1]});
      }
    }
